...
import menus.game_over_eng as over
import gameloop.game as loop
leveys, korkeus = 800, 800
naytto = pygame.display.set_mode((leveys, korkeus))

def game_over(pisteet, keratyt, missatut, tuhotut, ammukset):
        ...
        for tapahtuma in pygame.event.get():
            if tapahtuma.type == pygame.KEYDOWN:
                ...
                if tapahtuma.key == pygame.K_F1:
                    over.game_over_eng(pisteet, keratyt, missatut, tuhotut, ammukset)
            if tapahtuma.type == pygame.QUIT:
                exit()

        naytto.fill((20, 0, 0))
        game_over_tekstit.tulosta_tahdet()
        game_over_tekstit.game_over_teksti(pisteet, keratyt, missatut, tuhotut, ammukset)
        pygame.display.flip()
        kello.tick(fps)